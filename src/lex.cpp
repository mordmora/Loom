...
static constexpr std::size_t keywords_count =
...
#define RELEASE_COMPOUND_SYMBOL(T)                                             \
  do {                                                                         \
    current++;                                                                 \
    pos++;                                                                     \
    col++;                                                                     \
...
lexing:
  ...
  if (type != TokenType::None) {
    ...
    if (pos < buffer_len) {
      ...
      case '-':
        if (nc == '>')
          RELEASE_COMPOUND_SYMBOL(LArrow);
        break;
      case '=':
        if (nc == '=')
          RELEASE_COMPOUND_SYMBOL(EqEq);
        break;
      case '>':
        if (nc == '=')
          RELEASE_COMPOUND_SYMBOL(GreatOe);
        if (nc == '>')
          RELEASE_COMPOUND_SYMBOL(RShift);
        break;
      case '<':
        if (nc == '=')
          RELEASE_COMPOUND_SYMBOL(LessOe);
        if (nc == '<')
          RELEASE_COMPOUND_SYMBOL(LShift);
        break;
      case '.':
        if (nc == '.')
          RELEASE_COMPOUND_SYMBOL(DotDot);
        break;
      case '/':
        if (nc == '/') {
        ...
        if (nc == '*') {
          ...
          if (!closed) {
            std::cerr << "error: comentario abierto en linea " << comment_line
                      << " nunca fue cerrado\n";
            return Token(Str(start_p, pos - start_pos), start_pos,
                         pos - start_pos, TokenType::Inv);
          }
          goto lexing;
        }
        break;
